...
//            let cipherText = Cryptor(operation:.Encrypt, algorithm:.DES, options:.ECBMode, key:key, iv:Array<UInt8>()).update(arrayFrom(hexString: ivs[i]))?.final()
...
//            ("password", "salt", 16777216, 20, "eefe3d61cd4da4e4e9945b3d6ba2158c2634e984"),
    ...
    //    let key = zeroPad("thekey", blockSize: blockSize)
    //    let plainText = zeroPad(string: "username123", blockSize)
